...
// time against the models connected workers advertise. The known
// aliases are auto / fast / balanced / deep / max. Pass one as
// ChatRequest.Model, or call ListModes for the live set + descriptions.
// "max" selects the AB-MCTS deep-search tier; for that mode prefer the
// durable Solve / SolveAndWait clients, which run the search detached
// from the connection.
type Mode string
...
const (
	...
	ModeDeep     Mode = "deep"
	// ModeMax is the AB-MCTS deep-search tier. The orchestrator runs it
	// detached from the request connection — see Client.Solve.
	ModeMax Mode = "max"
)

// Modes lists the known modes in display order (auto first).
var Modes = []Mode{ModeAuto, ModeFast, ModeBalanced, ModeDeep, ModeMax}
...
type Message struct {
	...
	// Parts, when non-empty, carries multimodal content (text + images) for
	// vision-capable models, mutually exclusive with Content. If set, the
	// message marshals with `content` as the OpenAI parts array; otherwise
	// `content` is the plain Content string (the original behaviour). On
	// unmarshal, an array-shaped `content` populates Parts and flattens its
	// text into Content so string-only readers still see the text.
	Parts []ContentPart `json:"-"`
}

// ContentPart is one element of a multimodal message content array, following
// the OpenAI chat content-parts shape. Type is "text" or "image_url".
...
// ImageURL carries an image for a vision-capable model. URL is either an https
// URL or a data URI ("data:image/png;base64,<...>"). Detail is the optional
// OpenAI fidelity hint ("low" / "high" / "auto").
...
// ImagePart returns an image content part from an https URL or data URI.
func ImagePart(url string) ContentPart {
	return ContentPart{Type: "image_url", ImageURL: &ImageURL{URL: url}}
}

// UserMessageWithParts builds a user message from multimodal content parts.
func UserMessageWithParts(parts ...ContentPart) Message {
	return Message{Role: "user", Parts: parts}
}

// MarshalJSON renders the message in the OpenAI wire shape. When Parts is set,
// `content` is emitted as a structured array (text + image_url) for multimodal
// models; otherwise `content` is the plain string, preserving the original
// single-string behaviour byte-for-byte.
...
// UnmarshalJSON accepts `content` as either a plain string (the common case) or
// an array of content parts (multimodal). An array populates Parts and also
// flattens its text segments into Content, so callers that only read Content
// still get the textual portion.
func (m *Message) UnmarshalJSON(data []byte) error {
	...
	if err := json.Unmarshal(raw.Content, &parts); err != nil {
		return fmt.Errorf("korai: message content is neither a string nor a parts array: %w", err)
	}
	m.Parts = parts
...
type StreamEvent struct {
	...
	// Usage is set on the terminal "content" event (the chunk carrying
	// finish_reason) when the orchestrator reports token totals on the
	// stream. Nil on every earlier chunk and when the worker's backend
